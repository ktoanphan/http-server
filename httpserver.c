...
void serve_file(int fd, char* path) {
  ...
  snprintf(content_length, 64, "%ld", st.st_size);
  http_start_response(fd, 200);
  http_send_header(fd, "Content-Type", http_get_mime_type(path));
  http_send_header(fd, "Content-Length", content_length);
  http_end_headers(fd);
	
  while((bytes_read = read(file_fd, &buffer[total_read], sizeof(buffer) - total_read)) > 0){
      total_read += bytes_read;
      while (bytes_written < total_read) {
        bytes_written += write(fd, &buffer[bytes_written], total_read - bytes_written);
      }
  ...
  if (path_check != 0) {
    ...
    if (S_ISREG(st.st_mode)) {
      serve_file(fd, path);
        ...
        char index_html[length];
        http_format_index(index_html, path);
        struct stat st1; 
        int path_check1 = stat(index_html, &st1);
        if (path_check1 == 0) {
            serve_file(fd, index_html);
        } else {
...
void init_thread_pool(int num_threads, void (*request_handler)(int)) {
  ...
  for (i = 0; i < num_threads; i++) {
    result = pthread_create(&thread[i], NULL, handle_clients, (void*) request_handler);
    if (result) {
...
void serve_forever(int* socket_number, void (*request_handler)(int)) {
  ...
  *socket_number = socket(PF_INET, SOCK_STREAM, 0);
  ...
  while (1) {
    ...
    int result; 
    pthread_t tid;
    result = pthread_create(&tid, NULL, request_handler, (void*) client_socket_number);
    if (result) {
      perror("error creating a new thread");
      exit(errno);
    } 
